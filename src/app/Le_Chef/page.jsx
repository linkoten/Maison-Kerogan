...
const LeChef = () => {
  ...
  const animateChildElements = () => {
    if (newsletterRef.current) {
      const childElements = newsletterRef.current.querySelectorAll("*");
      ...
      {data.map((item) => (
        <div key={item.id}>
          <div className="flex px-4 mb-10 space-y-4 grid  grid-cols-1 lg:grid-cols-3 lg:px-0  lg:space-y-0">
            ...
            >
              <h3
                className={`${merriweather.className} text-vert  pt-4 font-bold text-lg md:text-xl lg:text-2xl mb-6`}
              >
                {item.title}
              </h3>

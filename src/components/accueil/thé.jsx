...
const ThéExtrait = () => {
  const { data, loading, error } = useData("salonDeThes", [
    "journees",
    "horaires",
    "extrait",
    "id",
  ...
  const renderBlock = (extrait, journees, horaires, images) => {
    const textContent = (
      <div className="flex flex-col px-6 w-full border-rose border-2  shadow-rose shadow-lg  hover:shadow-xl hover:shadow-rose bg-terracotta text-white rounded-xl">
        <div className="flex w-full pt-4 mb-6 justify-between">
          <h3
            className={`${merriweather.className} text-rose pl-6 font-bold text-lg md:text-xl lg:text-2xl uppercase`}
          >
            {item.title}
          </h3>
          <Image
        ...
        <div className="flex-1 flex flex-col justify-end text-xs md:text-sm lg:text-md">
          ...
          <Separator className="bg-rose" />
          <Button className="my-4 bg-rose  border border-black  hover:brightness-110">

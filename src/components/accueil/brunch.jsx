...
const BrunchExtrait = () => {
  const item = restaurant; // Assuming we're using the first item from the data array

  const renderBlock = (extrait, journees, horaires, images) => {
    const textContent = (
      <div className="flex flex-col px-6 w-full border-ocre border-2  shadow-ocre shadow-lg  hover:shadow-xl hover:shadow-ocre bg-vert text-white rounded-xl">
        ...
        <div className="flex-1 flex flex-col justify-end text-xs md:text-sm lg:text-md">
          ...
          <Separator className="bg-ocre" />
          <div className=" text-md font-bold">{item.horaires}</div>
          <Separator className="bg-ocre" />
          <Button className="my-4 bg-ocre  border border-black  hover:brightness-110">

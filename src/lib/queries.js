export const tapas = {
  part1: "After Work",
  paragraphe1:
    "Pour bien terminer la journée, venez passer un moment décontracté et convivial avec une carte de tapas maison travaillée tout au long des saisons, des cocktails avec et sans alcool, entre classiques et créatifs, des bières craft et autres boissons soigneusement sélectionnées.",
  paragraphe2: "",
  extrait:
    "Le soir, Maison Kerogan vous invite à découvrir une approche bistronomique du partage avec des menus évoluant au fil des saisons. Notre chef propose une sélection de tapas à la pièce, alliant créativité et saveurs inédites, pour une expérience gourmande et surprenante. Pour les amateurs de convivialité, quelques belles planches à partager viennent compléter l’offre. Laissez-vous tenter et explorez une cuisine où chaque bouchée raconte une histoire.",
  id: "tapas",
  slug: "tapas",
  title: "After Work Tapas",
  images: [
    "https://i.ibb.co/BPnCSSv/0054-Maison-RESTAURATION-105.jpg",
    "https://i.ibb.co/XJZ1gpr/0107-Maison-Kerogan-107.jpg",
    "https://i.ibb.co/TT3jzMz/0035-Maison-Kerogan-35.jpg",
  ],
  journees: "Du Mercredi au Samedi ",
  horaires: "De 18h à 23h",
  part2: "Cocktail",
  part2Paragraphe1:
    "Nous vous proposons une sélection de cocktails classiques et originaux, ainsi que des créations sans alcool. Nos boissons sont préparées avec des ingrédients frais et des sirops maison, notre carte évolue régulièrement pour vous offrir de nouvelles saveurs.",
  part2Images: [
    "https://i.ibb.co/TT3jzMz/0035-Maison-Kerogan-35.jpg",
    "https://i.ibb.co/G935YJ4/0004-Maison-Kerogan-140.jpg",
  ],
  part2Journees: "Du Lundi au Vendredi",
  part2Horaires: "De 17h à 22h",
  part3: "Tapas",
  part3Paragraphe1:
    "Le soir, Maison Kerogan vous invite à découvrir une approche bistronomique du partage avec des menus évoluant au fil des saisons. Notre chef propose une sélection de tapas à la pièce, alliant créativité et saveurs inédites, pour une expérience gourmande et surprenante. Pour les amateurs de convivialité, quelques belles planches à partager viennent compléter l’offre. Laissez-vous tenter et explorez une cuisine où chaque bouchée raconte une histoire.",
  part3Images: [
    "https://i.ibb.co/KzcBjW2S/IMG-4371.jpg",
    "https://i.ibb.co/zT9VRPPK/IMG-4358.jpg",
    "https://i.ibb.co/DDPJn9K7/IMG-4361.jpg",
  ],
  part3Journees: "Du Lundi au Vendredi",
  part3Horaires: "De 17h à 22h",
};

export const thé = {
  part1: "Tea Time",
  paragraphe1:
    "Retrouvez chaque jour un choix de pâtisseries individuelles et de gâteaux à la découpe, fraîchement conçus tous les matins par Emily Keller. ",
  paragraphe2:
    "Elles sont à déguster toute l’après-midi, accompagnées de boissons chaudes.",
  extrait:
    "Retrouvez chaque jour un choix de pâtisseries individuelles et de gâteaux à la découpe, fraîchement conçus tous les matins par Emily Keller. Elles sont à déguster toute l’après-midi, accompagnées de boissons chaudes, ainsi que des desserts servis au restaurant.",
  id: "salondethe",
  slug: "salondethe",
  title: "Tea Time",
  images: ["https://i.ibb.co/ypnJc7B/0005-Maison-Kerogan-5.jpg"],
  journees: "Du Mercredi au Samedi",
  horaires: "14h à 18h",
  part2: "Barista",
  part2Paragraphe1:
    "Prenez le temps d’une pause gourmande avec notre sélection de lattés et de thés, soigneusement choisis pour évoluer au fil des saisons. Que vous soyez amateur de douceur lactée ou de notes parfumées, nous vous proposons des boissons réconfortantes, préparées avec attention.",
  part2Images: [
    "https://i.ibb.co/TT3jzMz/0035-Maison-Kerogan-35.jpg",
    "https://i.ibb.co/G935YJ4/0004-Maison-Kerogan-140.jpg",
    "https://i.ibb.co/ypnJc7B/0005-Maison-Kerogan-5.jpg",
    "https://i.ibb.co/FL2Qs04d/Barista2.jpg",
    "https://i.ibb.co/FkF2TJbr/Barista1.jpg",
  ],
};

export const restaurant = {
  part1: "Restauration",
  paragraphe1:
    "Situé au cœur du site de Creac'h Gwen, le restaurant  la maison Kerogan vous invite à découvrir un lieu unique où nature et convivialité se rencontrent. Installez-vous confortablement dans notre ambiance chaleureuse et profitez de la vue sur la rivière de l'Odet.",
  paragraphe2: "",
  extrait:
    "Le midi, découvrez la cuisine bistronomique de notre Chef Morgan Robin, dont la carte évolue chaque semaine. Une fusion de saveurs et textures, mettant en avant les produits locaux, avec une touche d’inventivité et de détails.",
  id: "Brunch",
  slug: "Brunch",
  title: "Le Restaurant",
  images: [
    "https://i.ibb.co/mRzWH68/0049-Maison-Kerogan-49.jpg",
    "https://i.ibb.co/rp5bcHz/image00004.jpg",
    "https://i.ibb.co/zbbDCQs/image00001.jpg",
  ],
  journees: "Du Lundi au Vendredi",
  horaires: "12h-14h",
  part2: "La Cuisine",
  part2Paragraphe1:
    "Le midi, découvrez la cuisine bistronomique de notre Chef Morgan Robin, dont la carte évolue chaque semaine.",
  part2Paragraphe2:
    "Une fusion de saveurs et textures, mettant en avant les produits locaux, avec une touche d’inventivité et de détails.",
  part2Images: [
    "https://i.ibb.co/6rWPC3g/image00017.jpg",
    "https://i.ibb.co/gS3z1KL/0077-Maison-Kerogan-77.jpg",
    "https://i.ibb.co/4RWggRF/0026-Maison-RESTAURATION-89.jpg",
    "https://i.ibb.co/G935YJ4/0004-Maison-Kerogan-140.jpg",
    "https://i.ibb.co/sVY7KB3/0048-Maison-LIEU-59.jpg",
    "https://i.ibb.co/Lh29MR0/0028-Maison-RESTAURATION-75.jpg",
    "https://i.ibb.co/NyGQz8Q/0062-Maison-Kerogan-81.jpg",
    "https://i.ibb.co/VvCPxC7/image00022.jpg",
    "https://i.ibb.co/Vjkb95V/image00024.jpg",
    "https://i.ibb.co/4mbqxyG/image00026.jpg",
    "https://i.ibb.co/TPSHgPr/image00030.jpg",
    "https://i.ibb.co/phDRXjX/image00031.jpg",
    ,
  ],
  part3: "La Patisserie",
  part3Paragraphe1:
    "Retrouvez chaque jour un choix de pâtisseries individuelles et de gâteaux à la découpe, fraîchement conçus tous les matins par Emily Keller. ",
  part3Paragraphe2:
    "Elles sont à déguster en dessert ou toute l’après-midi, accompagnées de boissons chaudes.",
  part3Images: [
    "https://i.ibb.co/L0QvZhm/0091-Maison-Kerogan-116.jpg",
    "https://i.ibb.co/GHpsHt9/0031-Maison-Kerogan-123.jpg",
    "https://i.ibb.co/M6Vchcn/0087-Maison-Kerogan-125.jpg",
  ],
};

export const location = {
  part1: "Evenementielle",
  paragraphe1:
    "Tout au long de l'année, notre restaurant vous accueille lors d'événements variés : des open airs musicaux en extérieur, des brunchs proposés chaque dimanche, et la mise à disposition de notre espace pour vos événements privés.",
  titreExtrait: "Evenementielle",
  extrait:
    "Tout au long de l'année, retrouvez nos événements, tels que des brunchs bistronomiques le dimanche et des open airs musicaux pendant l'été.",
  titreExtrait2: "Location Evenementielle",
  extrait2:
    "Vous pouvez aussi privatiser le lieu pour vos soirées d'anniversaire, séminaires ou mariages.",
  id: "Location",
  slug: "Location",
  title: "Événementielle / Location Événementielle",
  images: ["/Video Maison Kerogan.mp4"],
  journees: "Week-end",
  part2: "Brunch",
  part2Paragraphe1:
    "Le dimanche, sur réservation, Maison Kerogan vous propose un brunch bistronomique unique. Notre carte évolue à chaque édition pour vous faire découvrir de nouvelles associations de saveurs, mêlant douceur et générosité. Une expérience à savourer en toute tranquillité, dans une ambiance chaleureuse et conviviale, en famille ou entre amis.",

  part2Images: ["https://i.ibb.co/mFPcDZxN/Brunch-Photo.jpg"],
  part2Horaires: "Dimanche",
  part2Journees: "De 12h à 15h",
  part3: "Location",
  part3Paragraphe1:
    "Offrez à vos événements un cadre unique en privatisant Maison Kerogan. Que ce soit pour un anniversaire, un baptême, un séminaire ou même un mariage, notre restaurant, son jardin et sa terrasse sont à votre disposition pour créer un moment sur mesure. Nous vous accompagnons dans l’organisation pour faire de votre réception une expérience inoubliable.",
  part3Paragraphe2:
    "Pour toute demande, contactez-nous par mail à maison-kerogan@maison-kerogan.com.",
  part3Images: [
    "https://i.ibb.co/7bW5sPH/LOGO-WEB-1366x768-Plan-de-travail-1-copie-6.jpg",
  ],
};

export const histoireDuLieu = {
  paragraphe1:
    "La maison Kerogan est une belle demeure des années 70, située à Créach Gwen sur un magnifique parc donnant sur l’Odet. Ce restaurant, rénové avec soin, vous accueille dans une ambiance chaleureuse et conviviale.",
  paragraphe2: "",
  id: "histoireDuLieu",
  slug: "histoireDuLieu",
  title: "Histoire Du Lieu",
  images: [
    "https://i.ibb.co/mcgV9sP/0130-Maison-Kerogan-130.jpg",
    "https://i.ibb.co/tXLCK5J/0021-Maison-LIEU-120.jpg",
    "https://i.ibb.co/rw88x5p/0005-Maison-LIEU-139.jpg",
    "https://i.ibb.co/Ypz51M7/LIEU-0010.jpg",
    "https://i.ibb.co/4tXsZsh/LIEU-1151.jpg",
  ],
};
